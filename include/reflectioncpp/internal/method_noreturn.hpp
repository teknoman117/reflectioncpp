...
namespace reflectioncpp
{
	...
	class MethodImpl<T, R(Args...), typename std::enable_if<std::is_void<R>::value && !std::is_void<T>::value>::type> : public Invokable
	{
	public:
	...
	private:
	    ...
	    {
	        // Invoke the method
	        (instance->*methodPointer)(std::forward<Ts>(ts)...);
	    }
	...
	private:
	    ...
	    {
	        // Invoke the method
	        (*methodPointer)(std::forward<Ts>(ts)...);
	    }
	...
	public:
	    ...
	    boost::any Invoke() const override
	    {
			...
			_Invoke(parameter_range::iterator());
			return boost::any();
	    }
